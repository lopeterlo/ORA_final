...
def prepare_generator_batch(inp, target, start_letter=0, gpu=False):
    """
    Takes samples (a batch) and returns

    Inputs: samples, start_letter, cudaS
...
def batchwise_sample(gen, train_samples, idx, dictionary, num_samples, batch_size):
    """
    ...
    for i in range(0, num_samples, batch_size):
        samples.append( gen.sample(train_samples[idx[0] + i :idx[0] + i + batch_size], dictionary))
    return torch.cat(samples, 0)[:num_samples]


def batchwise_oracle_nll(gen, oracle, num_samples, batch_size, max_seq_len, summary_samples, start_letter=0, gpu=False):
    s = batchwise_sample(gen, num_samples, batch_size)
    perm = torch.randperm(summary_samples.size(0))
    idx = perm[:num_samples]
    samples = summary_samples[idx]
    oracle_nll = 0
    for i in range(0, num_samples, batch_size):
        inp, target = prepare_generator_batch(s[i:i+batch_size], samples[i: i+ batch_size], start_letter, gpu)
